...
const Voucher = ( { voucherItems , relatedOrders } : Props) => {
    ...
    return (
        <div className={`bg-[#F8E6E6] relative ${relatedOrders ? "w-70 h-100 pt-5" : "grow"} rounded-[7px] border border-voucherColor flex flex-col gap-1 px-4 py-3 overflow-y-auto `}>
            ...
            {voucherItems.length ? <div className="grow flex flex-col px-2 pt-4 gap-2">
                ...
                <div className="flex flex-col gap-1 py-1.5 border-t border-t-voucherColor ">
                    <div className="flex items-center justify-between mb-1 px-1 ">
                        <span className="w-[21%]" />
                        <p className={abyssinica.className + "  text-[clamp(8px,1vw,11px)] text-voucherColor w-[21%] text-center"}>Sub total</p>
                    ...
                    <div className="flex items-center justify-between px-1 ">
                        ...
                        <p className={abyssinica.className + "  text-[clamp(8px,1vw,11px)] text-voucherColor w-[10%] text-center"}>{company.taxPercentage}%</p>
                        <p className={abyssinica.className + "  text-[clamp(8px,1vw,11px)] text-voucherColor w-[21.5%] text-end"}>{getTotalPrice() * 1/100 * company.taxPercentage}</p>
                    </div>
                    <div className="flex items-center justify-between px-1 ">
                        <p className={abyssinica.className + "  text-[clamp(8px,1vw,11px)] text-voucherColor w-[21%] text-nowrap"}>Service Charge</p>
                        <p className={abyssinica.className + "  text-[clamp(8px,1vw,11px)] text-voucherColor w-[10%] text-center"}>{company.serviceChargePercentage}%</p>
                        <p className={abyssinica.className + "  text-[clamp(8px,1vw,11px)] text-voucherColor w-[21.5%] text-end"}>{getTotalPrice() * 1/100 * company.serviceChargePercentage}</p>
                    </div>
                    <div className="flex items-center justify-end mt-2">
                        <div className="flex justify-between items-center w-[62%] pt-2 border-t border-voucherColor px-1">
                            ...
                            <p className={abyssinica.className + "  text-[clamp(8px,1vw,11px)] text-voucherColor w-[40%] text-end"}>{getTotalPrice() + (getTotalPrice() * 1/100 * company.taxPercentage) + (getTotalPrice() * 1/100 * company.serviceChargePercentage) }</p>
                        </div>
                    </div>
            ...
            {relatedOrders && <p className="absolute top-0 right-0 py-1 px-2 rounded-bl-[7px] bg-primary text-cente text-secondary">{relatedOrders[0].status}</p>}
            {(relatedOrders && relatedOrders[0].spicyLevel )&& <p className="py-1 px-2 text-center bg-foodBorder text-secondary rounded-2xl">Spicy Level {relatedOrders[0].spicyLevel}</p>}
        </div>
    )
}
